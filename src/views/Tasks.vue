...
function addTask(for_whom, from_whom, description, deadline) {
    ...
    axios.post("https://organiser-backend.vercel.app/add_task", new_task).then(() => {
        return refreshData();
    });
...
function deleteTask(task_id) {
    show_spinner.value = true;
    axios.delete("https://organiser-backend.vercel.app/delete_task/" + task_id).then(() => {
        return refreshData();
    });
    show_spinner.value = false;
}
...
async function refreshData() {
    show_spinner.value = true;
    const response = await axios.get("https://organiser-backend.vercel.app/get_full_data_for_user/" + user_id.value);
